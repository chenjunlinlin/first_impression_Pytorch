...
def get_opt(cfg, model, opt_para):
    ...
    """
    if cfg.opt == "SGD":
        optimizer = torch.optim.SGD(model.parameters(), weight_decay = cfg.weight_decay, lr=cfg.lr, momentum=cfg.momentum)
    if cfg.opt == "Adam":
        optimizer = torch.optim.Adam(model.parameters(), weight_decay = cfg.weight_decay, lr=cfg.lr)
...
def get_scheduler(cfg, opt, epoch):
    ...
    if epoch is not None:
        if cfg.scheduler == "StepLR":
            scheduler = torch.optim.lr_scheduler.StepLR(optimizer=opt, step_size=50, gamma=0.9, last_epoch=epoch)
        if cfg.scheduler == "CosineAnnealingWarmRestarts":
            scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer=opt, T_0=5, T_mult=2, eta_min=0.001, last_epoch=epoch)
    else:
        if cfg.scheduler == "StepLR":
            scheduler = torch.optim.lr_scheduler.StepLR(optimizer=opt, step_size=50, gamma=0.9)
        if cfg.scheduler == "CosineAnnealingWarmRestarts":
            scheduler = torch.optim.lr_scheduler.CosineAnnealingWarmRestarts(optimizer=opt, T_0=5, T_mult=2, eta_min=0.001)
...
def get_cudainfo():
    available_cuda_devices = [torch.device(f'cuda:{i}') for i in range(torch.cuda.device_count())]
...
def get_checkpoint(cfg, exp_name):
    ...
    if os.path.exists(checkpoints_path):
        ...
        checkpoint_path = os.path.join(checkpoints_path, checkpoints_list[-1])
