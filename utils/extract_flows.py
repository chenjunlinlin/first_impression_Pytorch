...
def process(video_path, video_name, resource_path, add_flow_path):
    frames, _, _ = read_video(video_path)
    frames = frames.permute(0, 3, 1, 2)
    video_name = video_name.replace(".mp4", "")
    ...
    if "train" in video_path:
        destpath_flow = os.path.join(resource_path, add_flow_path[0])
    else:
        destpath_flow = os.path.join(resource_path, add_flow_path[1])
    for i, (img1, img2) in enumerate(zip(frames, frames[1:])):
    # Note: it would be faster to predict batches of flows instead of individual flows
        img1, img2 = preprocess(img1, img2)
        img1, img2 = torch.unsqueeze(img1, dim=0), torch.unsqueeze(img2, dim=0)
