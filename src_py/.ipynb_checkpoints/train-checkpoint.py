...
def train(epoch):
    model.train()
    train_loss = 0
    for video_feat, audio_feat, label, video_name in tqdm.tqdm(train_loader, desc='epoch:{}'.format(epoch)):
        if iscuda :
            video_feat, audio_feat, label = video_feat.cuda(), audio_feat.cuda(), label.cuda()
        optimizer.zero_grad()
        output = model(video_feat, audio_feat, video_name)
    ...
    writer.add_scalar("MACC", acc.sum() / 5, epoch)
...
def val(epoch):
    model.eval()
    val_loss = 0
    for video_feat, audio_feat, label, video_name in tqdm.tqdm(val_loader, desc='{}'.format(epoch)):
        if iscuda :
            video_feat, audio_feat, label = video_feat.cuda(), audio_feat.cuda(), label.cuda()
        output = model(video_feat, audio_feat, video_name)
        loss = criterion(output, label)
...
if __name__=='__main__' :
    ...
    writer.add_graph(model=model, input_to_model = (torch.rand(128,3, 6, 112, 112), torch.rand(128, 6, 136)))
    print('--------模型初始化---------')
    if pretrain == "True" and os.path.exit(best_model_save_dir):
    ...
    scheduler = torch.optim.lr_scheduler.StepLR(optimizer=optimizer, step_size=1, gamma=1e-4)
